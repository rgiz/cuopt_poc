...
def create_router(
    ...
    get_cuopt_url,       # kept to preserve signature (unused)
) -> APIRouter:
    ...
    def ensure_ready():
        ...
        # build LOC_META from current locations dataframe if present; fallback to {}
        loc_meta = {}
        try:
            import pandas as pd
            if "locations_df" in DATA and DATA["locations_df"] is not None:
                loc_meta = build_loc_meta_from_locations_csv(DATA["locations_df"])
        except Exception:
            pass
        return DATA, matrices, loc_meta
    ...
    def _build_trip_from_leg(e: Dict[str,Any], M) -> Dict[str,Any]:
        start_loc = str(e.get("from","")).upper()
        end_loc   = str(e.get("to","")).upper()
        i = M["loc2idx"].get(start_loc); j = M["loc2idx"].get(end_loc)
    ...
    def plan_and_solve_cascades(req: PlanSolveCascadeRequest, request: Request):
        ...
        while queue:
            ...
            wk, tmn, tmi, cands = generate_candidates(cand_req, DATA, M, cfg, LOC_META, load_sla_windows())
            ...
            if chosen.deadhead_miles:  bd["deadhead"] = float(chosen.deadhead_miles * deadhead_cpm)
            if chosen.overtime_minutes: bd["overtime"] = float(chosen.overtime_minutes * overtime_cpm)
    ...
    def plan_and_solve(req: PlanRequest, request: Request):
        ...
        wk, trip_minutes, trip_miles, cands = generate_candidates(req, DATA, M, cfg, LOC_META, SLA_WINDOWS)
        if not cands:
            base = float(cfg.get("outsourcing_base_cost", 200.0))
            cost = base + trip_miles * out_per_mile
            return {
                "weekday": wk, "trip_minutes": trip_minutes, "trip_miles": trip_miles,
                "objective_value": float(cost),
                "assignments": [{
                    "trip_id": f"NEW:{req.start_location}->{req.end_location}@{req.when_local}",
                    "type": "outsourced","driver_id": None,"candidate_id": "OUTSOURCE",
                    "delay_minutes": 0.0,"uses_emergency_rest": False,"deadhead_miles": 0.0,
                    "overtime_minutes": 0.0,"miles_delta": float(trip_miles),"cost": float(cost),
        ...
        return {
            ...
            "assignments": [{
                "trip_id": f"NEW:{req.start_location}->{req.end_location}@{req.when_local}",
                "type": "reassigned","driver_id": chosen.driver_id,"candidate_id": chosen.candidate_id,
                "delay_minutes": chosen.delay_minutes,"uses_emergency_rest": False,
                "deadhead_miles": chosen.deadhead_miles,"overtime_minutes": chosen.overtime_minutes,
                "miles_delta": chosen.miles_delta,"cost": float(cost),"cost_breakdown": bd,
            }],
