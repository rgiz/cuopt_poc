...
from src.plan.cascade_candidates import (
    CascadeCandidateOut,
    _apply_insertion_strategy_to_schedule,
...
def test_apply_insertion_strategy_enforces_return_to_home_base():
    before = [
        {
            "element_type": "START FACILITY",
            "is_travel": False,
            "from": "COVENTRY NATIONAL HUB",
            "to": "COVENTRY NATIONAL HUB",
            "start_min": 50,
            "end_min": 80,
            "priority": 3,
        },
    ...
    strategy = {
        "type": "leg_replacement",
        "target_index": 3,
        "target_element": before[3],
    }

    req = type("Req", (), {})()
    req.start_location = "MIDLANDS SUPER HUB"
    req.end_location = "BIRMINGHAM MAIL CENTRE"
    req.trip_minutes = 60
    req.priority = 2

    matrices = {
        "loc2idx": {
            "COVENTRY NATIONAL HUB": 0,
            "MIDLANDS SUPER HUB": 1,
            "BIRMINGHAM MAIL CENTRE": 2,
        },
        "time": np.array(
            [
                [0, 60, 80],
    ...
    structure = {"home_base": "COVENTRY NATIONAL HUB"}
    ...
    assert after[-1]["element_type"] == "END FACILITY"
    assert str(after[-1]["to"]).upper() == "COVENTRY NATIONAL HUB"
    assert any("RETURN_TO_BASE" in str(e.get("changes", "")) for e in after)


def test_terminal_closure_adds_offload_before_return_when_last_leg_loaded():
    rebuilt = [
        {
            "element_type": "TRAVEL",
            "is_travel": True,
    ...
    out = cascade_mod._append_terminal_closure_if_missing(
        rebuilt=rebuilt,
        original_schedule=original,
        matrices=matrices,
        change_prefix="TEST",
        preferred_terminal_location="COVENTRY NATIONAL HUB",
    )

    assert any("OFFLOAD_BEFORE_RETURN" in str(e.get("changes", "")) for e in out)
    assert any("RETURN_TO_BASE" in str(e.get("changes", "")) for e in out)
    assert out[-1]["element_type"] == "END FACILITY"
    assert str(out[-1]["to"]).upper() == "COVENTRY NATIONAL HUB"


def test_finalize_schedule_moves_end_facility_to_terminal_position():
    reconstructed = [
        {
            "element_type": "TRAVEL",
            "is_travel": True,
    ...
    out = cascade_mod._finalize_schedule(reconstructed)

    assert out[-1]["element_type"] == "END FACILITY"
    assert out[-1]["start_min"] == 730
    assert "END_FACILITY_MOVED_TO_END" in str(out[-1].get("changes", ""))
