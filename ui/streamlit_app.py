...
BACKEND = os.getenv("API_BASE_URL", "http://backend:8000")

st.title("Dynamic Vehicle Rescheduler (cuOpt)")

with st.form("baseline"):
    rsl_path = st.text_input("RSL path", "data/sample/rsl.csv")
    loc_path = st.text_input("Locations path", "data/sample/locations.csv")
    submitted = st.form_submit_button("Build Baseline")
    if submitted:
        res = requests.post(f"{BACKEND}/solve/baseline", json={
            "rsl_path": rsl_path,
...
st.header("Insert New Load")
pickup = st.text_input("Pickup location_id")
drop = st.text_input("Drop location_id (optional)", "")
tw = st.text_input("Time window [start,end]", "480,540")
svc = st.number_input("Service time (min)", value=30)
prio = st.slider("Priority (1=highest)", 1, 5, 3)

if st.button("Evaluate Insertion"):
    start, end = [int(x.strip()) for x in tw.split(",")]
    res = requests.post(f"{BACKEND}/solve/insert", json={
        "pickup_location_id": pickup,
        "drop_location_id": drop or None,
